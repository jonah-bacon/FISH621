...
# Lower (5%) bound
lb.N_hat.5 <- trial.N[max(which(cumProb.trial.N<=0.05))]
# Cumulative probability at this level
lb.N_hat.cumProb.5 <-  cumProb.trial.N[max(which(cumProb.trial.N<=0.05))]

# Upper (95%) bound
ub.N_hat.95 <- trial.N[max(which(cumProb.trial.N<=0.95))]
# Cumulative probability at this level
ub.N_hat.cumProb.95 <-  cumProb.trial.N[max(which(cumProb.trial.N<=0.95))]

# Lower (25%) bound
lb.N_hat.25 <- trial.N[max(which(cumProb.trial.N<=0.25))]
# Cumulative probability at this level
lb.N_hat.cumProb.25 <-  cumProb.trial.N[max(which(cumProb.trial.N<=0.25))]

# Upper (75%) bound
ub.N_hat.75 <- trial.N[max(which(cumProb.trial.N<=0.75))]
# Cumulative probability at this level
ub.N_hat.cumProb.75 <-  cumProb.trial.N[max(which(cumProb.trial.N<=0.75))]
...
plot(x=trial.N, y=prob.trial.N, type="l", col="blue",
     ...
     ylab="Cumulative of Alternative Abundances",
     xlim=c(250,400),
     main="Approximate 90% CI")
grid()
polygon(x=trial.N, y=prob.trial.N, col=rgb(0,1,1, alpha=0.25))

lb.N_hat.prob.5 <- dbinom(x=n, size=lb.N_hat.5, prob=p)
lb.N_hat.prob.5

ub.N_hat.prob.95 <- dbinom(x=n, size=ub.N_hat.95, prob=p)
ub.N_hat.prob.95

# Upper 90% CI
segments(x0=ub.N_hat.95, y0=-1, 
         x1=ub.N_hat.95, y1=ub.N_hat.prob.95,
         col="red", lwd=2)
# Lower 90% CI
segments(x0=lb.N_hat.5, y0=-1, 
         x1=lb.N_hat.5, y1=lb.N_hat.prob.5,
         col="red", lwd=2)

# Shade this region.
polygon(x=c(lb.N_hat.5,lb.N_hat.5:ub.N_hat.95,ub.N_hat.95),
        y=c(0,dbinom(x=n, size=lb.N_hat.5:ub.N_hat.95, prob=p),0),
        col=rgb(1,0,0, alpha=0.2))
...
plot(x=trial.N, y=prob.trial.N, type="l", col="blue",
     ...
     ylab="Cumulative of Alternative Abundances",
     xlim=c(250,400),
     main="Approximate 50% CI")
grid()
polygon(x=trial.N, y=prob.trial.N, col=rgb(0,1,1, alpha=0.25))

lb.N_hat.prob.25 <- dbinom(x=n, size=lb.N_hat.25, prob=p)

ub.N_hat.prob.75 <- dbinom(x=n, size=ub.N_hat.75, prob=p)

# Upper 50% CI
segments(x0=ub.N_hat.75, y0=-1, 
         x1=ub.N_hat.75, y1=ub.N_hat.prob.75,
         col="red", lwd=2)
# Lower 50% CI
segments(x0=lb.N_hat.25, y0=-1, 
         x1=lb.N_hat.25, y1=lb.N_hat.prob.25,
         col="red", lwd=2)

# Shade this region.
polygon(x=c(lb.N_hat.25,lb.N_hat.25:ub.N_hat.75,ub.N_hat.75),
        y=c(0,dbinom(x=n, size=lb.N_hat.25:ub.N_hat.75, prob=p),0),
        col=rgb(1,0,0, alpha=0.2))
...
vec.n1 <- c(1000, 1000, 100, 100, 100)
vec.n2 <- c(100, 1000, 1000, 1000, 1e4)
vec.m2 <- c(20, 200, 10, 50, 50)

abund.est.matrix <- matrix(data = NA, nrow = 5, ncol = 3)
colnames(abund.est.matrix) <- c("pete", "chap", "bailey")

uncertainty.est.matrix <- matrix(data = NA, nrow = 5, ncol = 3)
colnames(uncertainty.est.matrix) <- c("pete", "chap", "bailey")

for (i in 1:5) {
  N_pete <- (vec.n1[i]*vec.n2[i])/vec.m2[i]
  var_m2_pete <- vec.m2[i]*(N_pete/(N_pete-1))*(1-(vec.n1[i]/N_pete))*(1-(vec.n2[i]/N_pete))
  N_chap <- ((vec.n1[i]+1)*(vec.n2[i]+1))/(vec.m2[i]+1) - 1
  var_N_chap <- ((vec.n1[i]+1)*(vec.n2[i]+1)*(vec.n1[i]-vec.m2[i])*(vec.n2[i]-vec.m2[i])) / ((vec.m2[i]+1)^2 * (vec.m2[i]+2))
  N_bailey <- (vec.n1[i]*(vec.n2[i]+1)) / (vec.m2[i]+1)
  var_N_bailey <- (vec.n1[i]^2 * (vec.n2[i]+1) * (vec.n2[i]-vec.m2[i])) / ((vec.m2[i]+1)^2 * (vec.m2[i]+2))
  abund.est.matrix[i,] <- c(N_pete, N_chap, N_bailey)
  uncertainty.est.matrix[i,] <- c(var_m2_pete, var_N_chap, var_N_bailey)
}

abund.est.matrix
uncertainty.est.matrix
...
trial.N_hat <- 3000:11000

# Calculate Chapman profile & plot
trial.N_hat.prob <- dhyper(x=m2, m=n1, n=trial.N_hat-n1, k=n2)

plot(x=trial.N_hat, y=trial.N_hat.prob, type="l",
     xlab="Trial Population Sizes (N_hat)",
     ylab="Probability of True Value",
     main="Hypergeometric (m2=50)")
polygon(x=trial.N_hat, y=trial.N_hat.prob, col=rgb(0,1,1, alpha=0.25))

# Calculate Chapman estimate and standard error of estimate
N_hat.chap <- ((n1+1)*(n2+1))/(m2+1) - 1
var_N_chap <- ((n1+1)*(n2+1)*(n1-m2)*(n2-m2)) / ((m2+1)^2 * (m2+2))
se_N_chap <- sqrt(var_N_chap)

# Add lines for Chapman estimator & 95% CI
abline(v=N_hat.chap, col="red")
abline(v=N_hat.chap + 1.96*se_N_chap, col = "darkgreen")
abline(v=N_hat.chap - 1.96*se_N_chap, col = "darkgreen")

# Calculate Bailey profile & plot
trial.N_hat.prob_bailey <- dbinom(x=m2, size=n2, prob=n1/trial.N_hat)

plot(x=trial.N_hat, y=trial.N_hat.prob_bailey, type="l",
     xlab="Trial Population Sizes (N_hat)",
     ylab="Probability of True Value",
     main="Bailey's Binomial Model (m2=50)")
polygon(x=trial.N_hat, y=trial.N_hat.prob_bailey, col=rgb(0,1,1, alpha=0.25))

# Calculate Bailey Estimator & standard error of estimator
N_hat.bailey <- (n1*(n2+1)) / (m2+1)
var_N_bailey <- (n1^2 * (n2+1) * (n2-m2)) / ((m2+1)^2 * (m2+2))
se_N_bailey <- sqrt(var_N_bailey)

# Add lines for Bailey estimator & 95% CI
abline(v=N_hat.bailey, col="red")
abline(v=N_hat.bailey + 1.96*se_N_bailey, col = "darkgreen")
abline(v=N_hat.bailey - 1.96*se_N_bailey, col = "darkgreen")
...
plot(x=trial.N_hat, y=trial.N_hat.prob_bailey, type="l",
     xlab="Trial Population Sizes (N_hat)",
     ylab="Probability of True Value",
     main="Bailey's Binomial Model (m2=100)")
polygon(x=trial.N_hat, y=trial.N_hat.prob_bailey, col=rgb(0,1,1, alpha=0.25))
...
plot(x=trial.N_hat, y=trial.N_hat.prob_bailey, type="l",
     xlab="Trial Population Sizes (N_hat)",
     ylab="Probability of True Value",
     main="Bailey's Binomial Model (m2=400)")
polygon(x=trial.N_hat, y=trial.N_hat.prob_bailey, col=rgb(0,1,1, alpha=0.25))
...
With n1 = 500 and m2 = 500 -> then n2 = N and there is no variation in the estimate.
# n1 <- 500
# n2 <- 600
# m2 <- 500
# N_hat.pete <- (n1*n2)/m2
...
cost.mat <- matrix(data = NA, nrow = length(n1), ncol = length(n2))
rownames(cost.mat) <- n1
colnames(cost.mat) <- n2

chap.CV.mat <- matrix(data = NA, nrow = length(n1), ncol = length(n2))
rownames(chap.CV.mat) <- n1
colnames(chap.CV.mat) <- n2

bailey.CV.mat <- matrix(data = NA, nrow = length(n1), ncol = length(n2))
rownames(bailey.CV.mat) <- n1
colnames(bailey.CV.mat) <- n2
...
for (i in 1:length(n1)) {
  ...
  for (j in 1:length(n2)) {
    ...
    temp.n1 <- n1[i]
    temp.n2 <- n2[j]
    m2 <- temp.n1*temp.n2/1000
    
    cost.mat[i,j] <- temp.n1*2 + temp.n2*1
    
    chap.est <- ((temp.n1+1)*(temp.n2+1))/(m2+1) - 1
    var.chap.est <- ((temp.n1+1)*(temp.n2+1)*(temp.n1-m2)*(temp.n2-m2)) / ((m2+1)^2 * (m2+2))
    chap.CV.mat[i,j] <- sqrt(var.chap.est)/chap.est
    
    bailey.est <- (temp.n1*(temp.n2+1)) / (m2+1)
    var.bailey.est <- (temp.n1^2 * (temp.n2+1) * (temp.n2-m2)) / ((m2+1)^2 * (m2+2))
    bailey.CV.mat[i,j] <- sqrt(var.bailey.est)/bailey.est
  }
